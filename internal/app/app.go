...
import (
	...
	"github.com/lacsar712/slagquen/internal/granbed"
	"github.com/lacsar712/slagquen/internal/quenloop"
	"github.com/lacsar712/slagquen/internal/fsm"
	"github.com/lacsar712/slagquen/internal/interlock"
...
type App struct {
	...
	plant        *quenloop.QuenchPlant
	towerFSM     *fsm.TowerFSM
	zones        *quenloop.ZoneTable
	...
	granbed      *granbed.Controller
	holdMgr      *slagtemp.HoldWindowManager
	holdEval     *slagtemp.HoldWindowEvaluator
	gradAudit    *slagtemp.OrderedGradientValidator
	guard        *interlock.Guard
	dryFSM       *fsm.SlagFSM
	scheduler    *clock.SegmentScheduler
	avgWindow    *clock.QuenchWindow
...
func New(cfg config.Config) (*App, error) {
	...
	zones, err := quenloop.NewZoneTable(towerID, cfg.ZoneCount, plenumID, cfg)
	if err != nil {
		return nil, err
	}
	plant := quenloop.NewQuenchPlant(cfg, clk, mem)
	...
	for i, z := range zones.Zones() {
		...
		plant.RegisterSensor(slagtemp.NewSensor(sensorID, z.Zone))
		targets = append(targets, cfg.TargetMoistPct)
	}
	if err := plant.InitProfile(zoneIDs, targets); err != nil {
		return nil, err
	}
	ventBank, err := granbed.NewVentBank(zoneIDs, "vent-damper")
	if err != nil {
		return nil, err
	}
	...
	holdMgr := slagtemp.NewHoldWindowManager()
	a := &App{
		...
		gradAudit:    slagtemp.NewOrderedGradientValidator(zoneIDs, cfg.MaxGradientDeltaPct, 0.5),
		guard:        interlock.NewGuard(guardPairs),
	}
	a.granbed = initDehumidController(ventBank, cfg.MaxGradientDeltaPct, func(readings []model.MoistureReading) float64 {
	...
	if pc, ok := a.clk.(*clock.ProcessClock); ok {
		...
		a.avgWindow = clock.NewQuenchWindow(a.clk, time.Duration(cfg.QuenchWindowMinutes)*time.Minute)
	}
	a.dryFSM = fsm.NewSlagFSM(towerID, a.onDryTransition)
	fsm.RegisterDryHeatHook(a.dryFSM.Hooks())
	a.dryRamp = NewQuenchRamp(a.clk, cfg.ProcessTick(), cfg.QuenchRampSteps)
