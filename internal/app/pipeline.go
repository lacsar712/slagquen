...
func (a *App) observeZoneMoisture(ctx context.Context) error {
	...
	if err := a.granbed.ApplyCycle(ctx, readings); err != nil {
		return err
	}
	return a.granbed.ValidateGradient(readings)
}
...
func (a *App) routeSummaryLine(plenum model.PlenumID) string {
	...
	return fmt.Sprintf("routes=%d active_vents=%d fan_load=%.0f",
		summary.RouteCount, a.granbed.Bank().ActiveCount(), a.stager.CurrentLoad())
}
...
func initHoldEvaluator(profile *slagtemp.ProfileManager, windows *slagtemp.HoldWindowManager) *slagtemp.HoldWindowEvaluator {
	return slagtemp.NewHoldWindowEvaluator(profile, windows)
}

func initDehumidController(bank *granbed.VentBank, maxDelta float64, gradientFn func([]model.MoistureReading) float64) *granbed.Controller {
	c := granbed.NewController(bank, maxDelta)
